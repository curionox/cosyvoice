...
async def inference_instruct_sft(
    ...
    if has_instruct:
        ...
        model_output = cosyvoice.inference_instruct_sft(final_tts_text, spk_id, clean_instruct, stream=stream, speed=speed)
    elif is_v3:
...
async def inference_instruct2(
    ...
    # --- 关键修复：格式化指令文本 ---
    # 清理并确保 <|endofprompt|> 标记存在
    clean_instruct = instruct_text.strip().replace('\n', ' ')
    if not clean_instruct:
        clean_instruct = "Neutral"
    if '<|endofprompt|>' not in clean_instruct:
        clean_instruct = f"{clean_instruct}<|endofprompt|>"

    # --- 关键修复：在文本前加换行符，物理隔离指令和文本 ---
    # 这可以重置注意力机制的位置偏差，防止提示词泄露
    final_tts_text = f"\n{tts_text.strip()}"
...
async def inference_emotion_clone(
    ...
    用法：
    - spk_id: 你的音色ID（使用预训练音色）
    - emotion_prompt: 情感描述（如 "用愤怒的语气说"、"请非常开心地说一句话"）

    示例：
    curl -X POST http://localhost:9234/inference_emotion_clone \
      -F "tts_text=记忆想是倒在掌心的水" \
      -F "spk_id=17" \
      -F "emotion_prompt=用愤怒的语气说" \
      -F "format=wav" \
      --output output.wav

    支持的情感指令参考 cosyvoice/utils/common.py：
    - 请非常开心地说一句话
    - 请非常伤心地说一句话
    - 请非常生气地说一句话
    - 请用尽可能快地语速说一句话
    ...
    if clean_prompt:
        if '<|endofprompt|>' not in clean_prompt:
    ...
    print(f"预训练音色+情感 - 文本: {tts_text[:30]}..., 音色: {spk_id}, 情感: {clean_prompt}")

    # 使用 inference_instruct2，传入 spk_id 作为 zero_shot_spk_id
    # prompt_wav 传 None，会使用 spk_id 的预训练音色
    model_output = cosyvoice.inference_instruct2(
        final_tts_text,
        instruct,
        None,  # 不需要上传音频
    ...
    return StreamingResponse(
        ...
        media_type="audio/wav",
        headers={
            "Content-Disposition": "attachment; filename=emotion_output.wav",
            "X-Seed-Used": str(seed)
        }
    )
